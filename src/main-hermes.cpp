...
Adafruit_NeoPixel pixels = Adafruit_NeoPixel(N_PIXELS, PIN_LEDS, NEO_RGBW);
...
public:
  ...
  void update_direct_rgb(ControlInput input)
  {
    for (int i = 0; i < N_PIXELS; i++)
    {
      pixels.setPixelColor(i, input.color_2.r, input.color_2.g, input.color_2.b);
    }
    pixels.show();
  }
  ...
  void update_direct_rgb_pulsing(ControlInput input)
  {
    ...
    for (int i = 0; i < N_PIXELS; i++)
    {
      ...
      pixels.setPixelColor(i, scale * input.color_2.r, scale * input.color_2.g, scale * input.color_2.b);
    }
    pixels.show();
  }
  ...
  {
    uint32_t c = pixels.ColorHSV(hue, 255, value);
    c = pixels.gamma32(c);
